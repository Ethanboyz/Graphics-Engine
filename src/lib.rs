...
struct Vertex {
    ...
    tex_coords: [f32; 2], // Passed to the texture sampler
}
...
struct InstanceRaw {
    // We can use this to convert Instance data into a matrix
    model: [[f32; 4]; 4],
}
...
impl State {
    async fn new(window: Arc<Window>) -> anyhow::Result<State> {
        ...
        let (device, queue) = adapter
            .request_device(&wgpu::DeviceDescriptor {
                ...
                // Limit for WebGL2 browser support
        ...
        let camera = Camera {
            ...
            up: cgmath::Vector3::unit_y(), // Specify which way is up
        ...
        let render_pipeline = device.create_render_pipeline(&wgpu::RenderPipelineDescriptor {
            ...
            vertex: wgpu::VertexState {
                ...
                buffers: &[Vertex::desc(), InstanceRaw::desc()], // Include vertex and instance buffers in pipeline
        ...
        let instances = (0..NUM_INSTANCES_PER_ROW)
            .flat_map(|z| {
                (0..NUM_INSTANCES_PER_ROW).map(move |x| {
                    ...
                    // So objects at (0, 0, 0) won't get scaled to zero as Quaternions can affect scale if created improperly
    ...
    pub fn render(&mut self) -> Result<(), wgpu::SurfaceError> {
...

        {
            ...
            render_pass.draw_indexed(0..self.num_indices, 0, 0..self.instances.len() as _); // Draw using num_indices vertices
        }
...
impl ApplicationHandler<State> for App {
    // Define window attributes, create the window and a future for state
    fn resumed(&mut self, event_loop: &ActiveEventLoop) {
        ...
        #[cfg(not(target_arch = "wasm32"))]
        {
            // If we are not on web, use pollster to await the future
    ...
    // For where the state future lands
    #[allow(unused_mut)]
    fn user_event(&mut self, _event_loop: &ActiveEventLoop, mut event: State) {
        // Where proxy.send_event() ends up
        #[cfg(target_arch = "wasm32")]
        {
